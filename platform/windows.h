...
{
	return CreateFile(file, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING,
					  FILE_ATTRIBUTE_NORMAL, NULL);
}
static void file_close(File f) { CloseHandle(f); }
static uint64_t file_size(File f) { return GetFileSize(f, NULL); }
static void socket_sendfile(Socket s, File f)
{
	TransmitFile(s, f, 0, 0, NULL, NULL, TF_DISCONNECT);
}